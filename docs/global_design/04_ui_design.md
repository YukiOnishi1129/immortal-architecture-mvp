...
- **ノート一覧**：/notes?q=検索語&status=draft|publish&page=1
  - 検索対象＝タイトル＋各セクションの**入力文字列**（セクション名は含めない）
- **テンプレ一覧**：/templates?q=検索語&field=項目名&page=1
  - MVPはqでテンプレ名・項目名の部分一致をまとめて検索
...
| 入力 | **各セクション（文字入力欄）** | テンプレに基づく項目（Problem／原因／対策／…）<br>※ Problem／Decision 必須、Decision は最大3行 |
...
| 入力 | **タイトル**（必須） | 既存タイトル編集 |
| 入力 | **各セクション（文字入力欄）** | 既存内容編集（Problem／Decision ルール適用） |
